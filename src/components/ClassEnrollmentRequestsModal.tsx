...
import { X, GraduationCap, User, Calendar, Clock, CheckCircle, XCircle, MessageSquare } from 'lucide-react';
import { ClassEnrollmentRequest } from '../types/Class';
import { useClassEnrollment } from '../contexts/ClassEnrollmentContext';

interface ClassEnrollmentRequestsModalProps {
  isOpen: boolean;
  onClose: () => void;
  userRole: 'admin' | 'staff' | 'student';
}

export default function ClassEnrollmentRequestsModal({ isOpen, onClose, userRole }: ClassEnrollmentRequestsModalProps) {
  const { 
    studentRequests, 
    pendingRequests, 
  ...
  }, [isOpen, userRole, fetchStudentRequests, fetchPendingRequests]);
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      await approveEnrollmentRequest(requestId, adminNotes[requestId]);
  ...
  const handleReject = async (requestId: string) => {
    setProcessingRequestId(requestId);
    try {
      await rejectEnrollmentRequest(requestId, adminNotes[requestId]);
      setAdminNotes(prev => ({ ...prev, [requestId]: '' }));
    } catch (error) {
  ...
  const requestsToShow = userRole === 'admin' ? pendingRequests : studentRequests;
  const title = userRole === 'admin' ? 'Pending Class Enrollment Requests' : 'My Class Enrollment Requests';
            ...
            <div>
              ...
              <p className="text-gray-600">
                {userRole === 'admin' 
                  ...
                  : 'Track your class enrollment requests'
                }
              </p>
            </div>
          </div>
          ...
          {loading ? (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-medium text-gray-900">{request.class?.name}</h4>
                          <p className="text-sm text-gray-600">{request.class?.subject} • {request.class?.level}</p>
                          <p className="text-xs text-gray-500">
                            {request.class?.currentStudents}/{request.class?.maxStudents} students
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {userRole === 'admin' && request.student && (
                            <p className="text-sm text-gray-600">
                              {request.student.name} {request.student.lastName}
                            </p>
                          )}
                          {request.student?.educationLevel && (
                            <p className="text-xs text-gray-500">{request.student.educationLevel}</p>
                          )}
                        </div>
                      </div>
                  ...
                  {request.notes && (
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <p className="text-sm text-gray-600">
                        <span className="font-medium">Student Notes:</span> {request.notes}
                      </p>
                    </div>
                  )}
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          disabled={processingRequestId === request.id || (request.class && request.class.currentStudents >= request.class.maxStudents)}
                      ...
                      {request.class && request.class.currentStudents >= request.class.maxStudents && (
                        <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700">
                          ⚠️ Cannot approve: Class is full
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {userRole === 'admin' ? 'No Pending Requests' : 'No Enrollment Requests'}
              </h3>
              <p className="text-gray-600">
                {userRole === 'admin' 
                  ? 'All class enrollment requests have been processed.'
                  : 'You haven\'t made any class enrollment requests yet.'
                }
              </p>
            </div>
          )}
